...
export const CreateTicketMobile: React.FC<CreateTicketMobileProps> = ({
    ...
    const tollFeeInputRef = useRef<HTMLInputElement>(null);
    ...
    // Helper to safely format dates to YYYY-MM-DDTHH:mm for input type="datetime-local"
        ...
        try {
            ...
            if (dateStr.includes('/')) {
                ...
                if (parts.length >= 3 && parts[2].length >= 4) {
                    d = new Date(`${parts[2].substring(0,4)}-${parts[1].padStart(2, '0')}-${parts[0].padStart(2, '0')}`);
                } else {
                    d = new Date(dateStr);
                }
            } else {
    ...
    const [formData, setFormData] = useState({
        dateStart: formatDateForInput(ticketToEdit?.dateStart),
        dateEnd: formatDateForInput(ticketToEdit?.dateEnd),
        ...
        tollFee: ticketToEdit?.tollFee || '',
        tollFeeImage: null as File | null,
        tollFeeImagePreview: ticketToEdit?.tollFeeImage || ''
    });
    ...
    const handleTollFeeImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            const file = e.target.files[0];
            try {
                const compressedBase64 = await compressImage(file);
                setFormData(prev => ({
                    ...prev,
                    tollFeeImage: file,
                    tollFeeImagePreview: compressedBase64
                }));
            } catch (error) {
                console.error("Compression failed", error);
                alert("Lỗi xử lý ảnh. Vui lòng thử lại.");
    ...
    const clearTollFeeImage = (e: React.MouseEvent) => {
        e.stopPropagation();
        setFormData(prev => ({
            ...prev,
            tollFeeImage: null,
        ...
        if (tollFeeInputRef.current) tollFeeInputRef.current.value = '';
    };

    const handleSubmit = async (status: 'PENDING' | 'DRAFT' | 'MỚI TẠO' | 'ĐANG VẬN CHUYỂN' | 'BÁO CÁO CS') => {
        // Validation for any status except DRAFT
        if (status !== 'DRAFT') {
            ...
            if (formData.nightStay) {
                ...
                if (!formData.nightStayDays || formData.nightStayDays <= 0) {
                    alert('Vui lòng nhập số đêm lưu trú hợp lệ (Lớn hơn 0)');
                    return;
                }
        ...
        const { containerImage, containerImagePreview, ...cleanFormData } = formData;

        if (ticketToEdit) {
            const updatedTicket: TransportTicket = {
                ...ticketToEdit,
                ...cleanFormData,
                dateStart: cleanFormData.dateStart && !cleanFormData.dateStart.endsWith('Z') ? new Date(cleanFormData.dateStart).toISOString() : cleanFormData.dateStart,
                dateEnd: cleanFormData.dateEnd && !cleanFormData.dateEnd.endsWith('Z') ? new Date(cleanFormData.dateEnd).toISOString() : cleanFormData.dateEnd,
                status,
                submittedToCS: status === 'PENDING' ? true : ticketToEdit.submittedToCS,
                submittedAt: status === 'PENDING' ? new Date().toISOString() : ticketToEdit.submittedAt,
                ...
                statusHistory: [
                    {
                        ...
                        action: status === 'DRAFT' ? 'Cập nhật bản nháp' : (status === 'PENDING' ? 'Gửi lại phiếu chờ duyệt' : 'Cập nhật thông tin')
                    },
                    ...(ticketToEdit.statusHistory || [])
                ]
            };
            setIsSubmitting(true);
            if (onUpdateSingleTicket) {
                onUpdateSingleTicket(updatedTicket)
                    .then(() => {
            ...
            const newTicket: TransportTicket = {
                ...
                dateEnd: cleanFormData.dateEnd && !cleanFormData.dateEnd.endsWith('Z') ? new Date(cleanFormData.dateEnd).toISOString() : cleanFormData.dateEnd,
                status,
                submittedToCS: status === 'PENDING' ? true : false,
                ...
                statusHistory: [
                    {
                        ...
                        timestamp: format(new Date(), 'HH:mm dd/MM/yy'),
                        user: user?.name || user?.username || 'Driver',
                        action: status === 'DRAFT' ? 'Lưu nháp' : 'Gửi phiếu cho CS duyệt'
                    }
                ]
            };
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300 pb-20">
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-1 gap-3">
                        <div>
                            <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Ngày bắt đầu (Tùy chọn giờ)</label>
                            <input
                                type="datetime-local"
                        ...
                        <div>
                            <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Ngày kết thúc (Tùy chọn giờ)</label>
                            <input
                                type="datetime-local"
            ...
            <div className="flex flex-col gap-3 pt-2 pb-8">
                {ticketToEdit && ticketToEdit.dispatchStatus !== 'COMPLETED' && ticketToEdit.status !== 'COMPLETED' ? (
                    // Cập nhật thông tin cho chuyến đang chạy
                    <div className="flex flex-col gap-3">
                        <button
                            onClick={() => handleSubmit(ticketToEdit.status as any || 'ĐANG VẬN CHUYỂN')}
                    ...
                    // Mới tạo hoặc Chuyến đã báo HOÀN THÀNH thì mới được gửi hoặc lưu nháp
                    <>
                        <button
