...
export const TicketDetailMobile: React.FC<TicketDetailMobileProps> = ({ ticket, onBack, onSubmitted }) => {
    ...
    const getStatusInfo = (status: string) => {
        switch (status) {
            ...
            case 'COMPLETED':
                return ticket.submittedToCS ? { color: 'text-orange-600', bg: 'bg-orange-50', icon: <Clock size={14} />, label: 'Chờ duyệt' } : { color: 'text-blue-600', bg: 'bg-blue-50', icon: <CheckCircle size={14} />, label: 'Chờ nộp chứng từ' };
                ...
                statusHistory: [
                    {
                        ...
                        user: ticket.driverName || 'Lái xe',
                        action: 'Gửi phiếu cho CS duyệt'
                    },
                    ...existingHistory
                ]
            });
