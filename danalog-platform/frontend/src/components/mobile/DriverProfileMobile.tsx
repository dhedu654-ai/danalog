...
import { User, Key, Save, Phone, Truck, CheckCircle, Clock, LogOut, Fuel } from 'lucide-react';
...
export const DriverProfileMobile: React.FC = () => {
    ...
    const [profileForm, setProfileForm] = useState({
        ...
        fuelCapacity: user?.fuelCapacity || '',
    });
                ...
                {activeTab === 'profile' ? (
                    <div className="space-y-4">
                        ...
                        <form onSubmit={handleProfileSubmit} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 space-y-4">
                            ...
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase tracking-wide block mb-1">Dung tích bình nhiên liệu (Lít)</label>
                                <div className="relative">
                                    <Fuel size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                                    <input 
                                        type="number"
                                        min="0"
                                        step="1"
                                        value={profileForm.fuelCapacity}
                                        onChange={e => setProfileForm({ ...profileForm, fuelCapacity: e.target.value })}
                                        placeholder="VD: 300"
                                        className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-sm font-bold text-slate-800 outline-none focus:ring-2 focus:ring-blue-500/50 font-mono"
                                    />
                                </div>
                            </div>
