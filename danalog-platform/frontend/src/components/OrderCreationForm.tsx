...
export function OrderCreationForm({ isOpen, onClose, routeConfigs, currentUser, onSuccess }: OrderCreationFormProps) {
    const [formData, setFormData] = useState({
        ...
        routeId: '',
        routeName: '',
        pickupDate: new Date(new Date().getTime() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16),
        deliveryDate: new Date(new Date().getTime() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16),
        containers: [
            { id: '20F', size: '20', fe: 'F', count: 0 },
            { id: '20E', size: '20', fe: 'E', count: 0 },
            { id: '40F', size: '40', fe: 'F', count: 1 },
            { id: '40E', size: '40', fe: 'E', count: 0 }
        ],
        notes: '',
        billNo: ''
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsSaving(true);
        try {
            const activeContainers = formData.containers.filter(c => (c.count || 0) > 0);
            const payload = {
                id: 'ORD-' + Date.now(),
                    ...
                    <form onSubmit={handleSubmit} className="px-8 py-8 space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm border-l-4 border-l-blue-500">
                            <div className="flex items-center justify-between mb-4">
                                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Cấu hình Container (Chọn nhanh loại & Số lượng)</h4>
                            </div>
                            
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                {formData.containers.map((container, index) => {
                                    const isActive = (container.count || 0) > 0;
                                    const isFull = container.fe === 'F';
                                    
                                    return (
                                        <div 
                                            key={container.id} 
                                            className={`p-3 rounded-xl border transition-all cursor-pointer ${isActive ? 'border-blue-500 bg-blue-50/50 shadow-sm' : 'border-slate-200 bg-white hover:border-slate-300'}`}
                                            onClick={() => {
                                                if (!isActive) {
                                        ...
                                        >
                                            <div className="flex justify-between items-center mb-3">
                                                <p className={`text-sm font-bold ${isActive ? 'text-blue-700' : 'text-slate-700'}`}>
                                                    {container.size}' {isFull ? 'Full' : 'Empty'}
                                                </p>
                                                {isActive && (
                                                    <button 
                                                        type="button"
                                            ...
                                            <div className="flex items-center justify-between bg-white rounded-lg border border-slate-200 p-1" onClick={e => e.stopPropagation()}>
                                                <button 
                                                    type="button"
                                                    disabled={!isActive || container.count <= 0}
                                                    onClick={() => {
                                                        const newC = [...formData.containers];
                                                        newC[index].count = Math.max(0, (container.count || 0) - 1);
                                                        setFormData({ ...formData, containers: newC });
                                                    }}
                                                    className="w-6 h-6 rounded bg-slate-100 flex items-center justify-center hover:bg-slate-200 disabled:opacity-30 text-xs font-bold"
                                                >
                                                    -
                                                </button>
                                                <input
                                                    type="number"
                                                    disabled={!isActive}
                                                    className="w-8 text-center bg-transparent text-sm font-bold text-blue-700 outline-none disabled:opacity-50"
                                                    value={container.count || 0}
                                                    onChange={e => {
                                                        const newC = [...formData.containers];
                                                        newC[index].count = parseInt(e.target.value) || 0;
                                                        setFormData({ ...formData, containers: newC });
                                                    }}
                                                />
                                                <button 
                                                    ...
                                                    onClick={() => {
                                                        const newC = [...formData.containers];
                                                        newC[index].count = (container.count || 0) + 1;
                                                        setFormData({ ...formData, containers: newC });
                                                    }}
                                                    className="w-6 h-6 rounded bg-blue-600 text-white flex items-center justify-center hover:bg-blue-700 disabled:opacity-30 shadow-sm text-xs font-bold"
                                                >
                                                    +
                                                </button>
                                            </div>
