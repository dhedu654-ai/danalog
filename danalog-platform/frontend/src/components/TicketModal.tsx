...
export function TicketModal({ ticket, isOpen, onClose, onSave, routeConfigs = [], isReadOnly, currentUser }: TicketModalProps) {
    ...
    const currentRouteConfig = useMemo(() => {
        if (!formData.route) return null;
        const candidates = safeRouteConfigs.filter(rc => rc && rc.routeName === formData.route);
        ...
        return candidates.find(rc => (rc.effectiveDate || '') <= ticketDate) || candidates[candidates.length - 1];
    }, [formData.route, safeRouteConfigs, formData.dateStart]);

    // Logic: Enforce night stay based on duration days & route config
    useEffect(() => {
        setFormData(prev => {
            const updates: Partial<TransportTicket> = {};
            ...
            // 2. City Location Enforcement based on active route config
            const willBeNightStay = updates.nightStay !== undefined ? updates.nightStay : prev.nightStay;
            if (willBeNightStay && currentRouteConfig) {
                const routeLoc = currentRouteConfig.nightStayLocation;
                if (routeLoc === 'INNER_CITY' && prev.cityStatus !== 'IN_CITY') {
                    updates.cityStatus = 'IN_CITY';
                }
                if (routeLoc === 'OUTER_CITY' && prev.cityStatus !== 'OUT_CITY') {
                    updates.cityStatus = 'OUT_CITY';
                }
            }
    ...
    }, [isSameDay, durationDays, currentRouteConfig]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const preparedData = { ...ticket, ...formData };
        if (formData.dateStart && !formData.dateStart.endsWith('Z')) {
            preparedData.dateStart = new Date(formData.dateStart).toISOString();
        }
        if (formData.dateEnd && !formData.dateEnd.endsWith('Z')) {
            preparedData.dateEnd = new Date(formData.dateEnd).toISOString();
        }
        if (onSave) onSave(preparedData as TransportTicket);
        onClose();
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 font-sans">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-slate-100 flex flex-col max-h-[95vh]">
                ...
                <div className="px-8 py-5 border-b border-slate-100 bg-white flex justify-between items-center shrink-0">
                    <div>
                        <h3 className="text-xl font-bold text-slate-800">{isReadOnly ? 'Chi Tiết Phiếu Công Tác' : 'Chỉnh Sửa Phiếu Công Tác'}</h3>
                        <p className="text-sm text-slate-500 mt-1">{isReadOnly ? 'Xem thông tin chi tiết của phiếu vận chuyển' : 'Cập nhật thông tin chi tiết cho phiếu vận chuyển'}</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto bg-slate-50/30">
                    <form onSubmit={handleSubmit} className="px-8 py-8 space-y-8">
                        ...
                        {ticket.statusHistory && ticket.statusHistory.length > 0 && (
                            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 shadow-inner mb-8">
                                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-4">Lịch sử phiếu (Timeline)</h4>
                                <div className="space-y-4">
                                    {ticket.statusHistory.map((h, i) => {
                                        const d = new Date(h.timestamp);
                                        const dateStr = isValid(d) ? format(d, 'dd/MM HH:mm') : (h.timestamp || 'N/A');
                                        return (
                                            <div key={i} className="flex gap-4">
                                                <div className="text-xs font-mono font-bold text-slate-400 w-24 shrink-0 mt-0.5 whitespace-nowrap">
                                                    {dateStr}
                                                </div>
                                                <div className="flex-1 relative pb-4 border-l-2 border-slate-200 pl-4 last:border-transparent last:pb-0">
                                                    <div className="absolute -left-[5px] top-1 w-2 h-2 rounded-full bg-blue-300"></div>
                                                    <div className="text-sm font-bold text-slate-700">{h.action} - {h.status}</div>
                                                    <div className="text-[11px] text-slate-500 uppercase font-semibold mt-0.5">Bởi: <span className="text-slate-800">{h.user || (h as any).actor || 'Hệ Thống'}</span></div>
                                                </div>
                                            </div>
                                        )
                                    })}
                        ...
                        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                {/* Editable: Date Start */}
                                <div>
                                    <label className="block text-xs font-semibold text-slate-500 mb-1.5 uppercase">Ngày bắt đầu (Tùy chọn giờ)</label>
                                    <input
                                        type="datetime-local"
                                        ...
                                        value={formData.dateStart && isValid(new Date(formData.dateStart)) ? format(new Date(formData.dateStart), "yyyy-MM-dd'T'HH:mm") : ''}
                                        onChange={e => handleChange('dateStart', e.target.value)}
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-slate-500 mb-1.5 uppercase">Ngày kết thúc (Tùy chọn giờ)</label>
                                    <input
                                        type="datetime-local"
                                        className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm font-medium hover:bg-slate-50"
                                        value={formData.dateEnd && isValid(new Date(formData.dateEnd)) ? format(new Date(formData.dateEnd), "yyyy-MM-dd'T'HH:mm") : ''}
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        {Array.from(new Set([...availableRoutes, formData.route].filter(Boolean))).map(r => (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
                                        ...
                                        <div className="col-span-12 md:col-span-5 grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <select
                                                    className={`w-full px-4 py-2.5 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer text-sm font-medium ${isSameDay ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : (!isSameDay && durationDays > 0 ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : '')}`}
                                                    value={formData.nightStay ? "YES" : "NO"}
                                                    onChange={e => handleChange('nightStay', e.target.value === "YES")}
                                                    disabled={!!isSameDay || (!isSameDay && durationDays > 0)}
                                                >
                                            ...
                                            {formData.nightStay && (
                                                <div className="animate-in fade-in zoom-in-95 duration-200 space-y-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className={`flex rounded-lg p-1 border ${currentRouteConfig?.nightStayLocation ? 'bg-slate-200 border-slate-300 opacity-80 pointer-events-none' : 'bg-slate-100 border-slate-200'}`}>
                                                            <button
                                                                type="button"
