...
export function ProfileApprovals({ currentUser, onUserUpdated }: ProfileApprovalsProps) {
    ...
    const fetchRequests = async () => {
        setLoading(true);
        try {
            // Fetch all requests - UI handles status filtering via statusFilter state
                            ...
                            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 bg-gradient-to-br from-slate-200 to-slate-300 rounded-full flex items-center justify-center text-slate-600 font-bold">
                                        {(req.fullName || req.username || '?').charAt(0)}
                                    </div>
                                    <div>
                                        <div className="font-bold text-slate-800">{req.fullName || req.username} <span className="text-xs font-mono text-slate-400 ml-1">({req.username})</span></div>
                                        <div className="text-xs text-slate-500 flex items-center gap-2">
                                            <span>Bộ phận: <b>{req.role}</b></span>
                                            <span>•</span>
                                            <span>Gửi lúc: {new Date(req.created_at || '').toLocaleString('vi-VN')}</span>
                                        </div>
                                    </div>
                                </div>
                                <div>
