...
import { ZONE_PRESETS } from '../constants';
...
export function RouteConfigModal({ config, isOpen, onClose, onSave, onRefresh, isNew, isReadOnly }: RouteConfigModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            // Route Configs for Cargo Types + Zones
            api.getRouteConfigs().then((configs: RouteConfig[]) => {
                ...
                types.add('VC_CONT');
                types.add('LUU_DEM');
                ZONE_PRESETS.forEach(z => zones.add(z));
                ...
                setExistingZones(Array.from(zones).sort());
            }).catch(() => { });
        }
    }, [isOpen]);
                        ...
                        <fieldset disabled={isReadOnly} className="contents">
                            ...
                            {activeTab === 'basic' && (
                                <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                                    <h4 className="text-lg font-bold text-slate-800 mb-4 border-b border-slate-100 pb-2">Thông tin tuyến đường</h4>
                                    <div className="grid grid-cols-2 gap-6">
                                        {/* Row 1: Point A & Point B */}
                                        <div className="col-span-2 md:col-span-1">
                                            <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Điểm đi (A)</label>
                                            <input
                                                type="text"
                                                ...
                                                onChange={e => {
                                                    handleChange('pointA', e.target.value);
                                                    if (formData.cargoType !== 'LUU_DEM') {
                                                        const newName = e.target.value && formData.pointB ? `${e.target.value} - ${formData.pointB}` : e.target.value || formData.pointB || '';
                                                        handleChange('routeName', newName);
                                                    }
                                                }}
                                                placeholder="VD: Cảng Tiên Sa"
                                            />
                                        </div>
                                        <div className="col-span-2 md:col-span-1">
                                            <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Điểm đến (B)</label>
                                            <input
                                                type="text"
                                                className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-medium text-slate-700 disabled:bg-slate-100 disabled:text-slate-500"
                                                value={formData.pointB || ''}
                                                onChange={e => {
                                                    handleChange('pointB', e.target.value);
                                                    if (formData.cargoType !== 'LUU_DEM') {
                                                        const newName = formData.pointA && e.target.value ? `${formData.pointA} - ${e.target.value}` : formData.pointA || e.target.value || '';
                                                        handleChange('routeName', newName);
                                                    }
                                                }}
                                                placeholder="VD: KCN Hòa Khánh"
                                            />
                                        </div>

                                        {/* Route Name (auto-generated or editable for special cases) */}
                                        <div className="col-span-2">
                                            <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Tên tuyến đường</label>
                                            {formData.cargoType === 'LUU_DEM' ? (
                                                ...
                                                <input
                                                    type="text"
                                                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none font-medium text-slate-700 disabled:bg-slate-100 disabled:text-slate-500"
                                                    value={formData.routeName || ''}
                                                    onChange={e => handleChange('routeName', e.target.value)}
                                                    placeholder="Tự tạo từ Điểm đi - Điểm đến"
                                                />
                                            )}
                                            <p className="text-[10px] text-slate-400 mt-1 italic">Tự động tạo từ Điểm đi và Điểm đến. Có thể chỉnh sửa nếu cần.</p>
                                        </div>

                                        {/* Row 2: Cargo Type & Customer */}
                                        <div className="col-span-2 md:col-span-1">
                                        ...
                                        {/* Row 3: KM & Zone */}
                                        <div className="col-span-2 md:col-span-1">
                                            <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Khoảng cách (KM)</label>
                                            <div className="relative">
                                                <input
                                                    type="number"
                                                    className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-medium text-slate-700 disabled:bg-slate-100 disabled:text-slate-500"
                                                    value={formData.km || 0}
                                                    onChange={e => handleChange('km', e.target.valueAsNumber || 0)}
                                                    min={0}
                                                />
                                        ...
                                        <div className="col-span-2 md:col-span-1">
                                            <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Vùng (Zone)</label>
                                            <input
                                                list="zones-list"
                                                className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-medium text-slate-700 disabled:bg-slate-100 disabled:text-slate-500"
                                                value={formData.zone || ''}
                                                onChange={e => handleChange('zone', e.target.value)}
                                                placeholder="Chọn hoặc nhập vùng..."
                                            />
                                            <datalist id="zones-list">
                                                {existingZones.map((z, idx) => (
                                                    <option key={`${z}-${idx}`} value={z} />
                                                ))}
                                            </datalist>
                                            <p className="text-[10px] text-slate-400 mt-1 italic">Nhập tên vùng mới nếu chưa có trong danh sách.</p>
                                        </div>
