                                ...
                                <tr key={user.username} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="p-4">
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${user.role === 'ADMIN' ? 'bg-purple-100 text-purple-700' :
                                            user.role === 'CS' ? 'bg-pink-100 text-pink-700' : 'bg-blue-100 text-blue-700'
                                            }`}>
                                            {user.role === 'DRIVER' ? 'Lái xe' :
                                                user.role === 'CS' ? 'CS (Văn phòng)' :
                                    ...
                                    <td className="p-4">
                                        {user.role === 'DRIVER' && user.licensePlate && (
                                            <div className="flex items-center gap-2 text-sm text-slate-600">
                                                <Truck size={16} />
                                                <span className="font-mono bg-slate-100 px-2 py-0.5 rounded">{formatLicensePlate(user.licensePlate)}</span>
                                            </div>
                                        )}
                                    </td>
                                    <td className="p-4">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            {formData.role === 'DRIVER' && (
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-1">Dung tích xe</label>
                                        <input
                                            type="text"
                                            disabled={!isAdmin}
                                            className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none disabled:bg-slate-50 disabled:text-slate-500"
