        ...
        <div className="max-w-4xl mx-auto py-6 space-y-6">
            <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-500">Tài khoản: <span className="font-mono text-slate-700">{currentUser?.username}</span> {currentUser?.employeeCode && `• Mã NV: ${currentUser.employeeCode}`} • Vai trò: <span className="font-medium text-blue-600">{currentUser?.role}</span></p>
                </div>
            </div>
                    ...
                    {pendingRequest && (
                        <div className="bg-amber-50 border-b border-amber-200 p-4 flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-amber-700 mt-1">Bạn đã gửi yêu cầu thay đổi thông tin vào <span className="font-medium">{new Date(pendingRequest.created_at || '').toLocaleString('vi-VN')}</span>. Vui lòng chờ duyệt viên xử lý trước khi có thể chỉnh sửa tiếp.</p>
