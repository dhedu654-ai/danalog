...
export default async function handler(req, res) {
    ...
    try {
        ...
        (tickets || []).forEach(t => {
             if (t.dispatchStatus === 'WAITING_AUTO') stats.waitingAuto++;
             ...
             if (t.status === 'IN_PROGRESS' || t.status === 'DISPATCHED') stats.inProgress++;
             if (t.status === 'COMPLETED' || t.status === 'APPROVED') stats.completed++;
        });
